...
This document describes how `IvanMurzak/Unity-AI-ProBuilder` signs its
`com.ivanmurzak.unity.mcp.probuilder` package so the warning no longer appears in
Unity 6.3+.
...
which downloads every release asset (the `.unitypackage` installer and the signed
...
```bash
gh secret set UPM_SERVICE_ACCOUNT_KEY_ID     --repo IvanMurzak/Unity-AI-ProBuilder
gh secret set UPM_SERVICE_ACCOUNT_KEY_SECRET --repo IvanMurzak/Unity-AI-ProBuilder
gh secret set UPM_ORG_ID                     --repo IvanMurzak/Unity-AI-ProBuilder
```

### 3. File the OpenUPM listing change

OpenUPM's package listing for `com.ivanmurzak.unity.mcp.probuilder` currently has
...
repository at `data/packages/com.ivanmurzak.unity.mcp.probuilder.yml`. Open a PR
there changing:
...
filename prefix rather than guessing from the asset list. The release also
ships the `.unitypackage` installer, and may add more `.tgz` assets later, so the
prefix guard prevents a future-breaking failure mode:

```yaml
githubReleaseAssetName: 'com.ivanmurzak.unity.mcp.probuilder-'
```
...
1. Go to the [release page](https://github.com/IvanMurzak/Unity-AI-ProBuilder/releases)
   for the new version and confirm a `com.ivanmurzak.unity.mcp.probuilder-<version>.tgz`
   asset is attached alongside the `.unitypackage`. The single-step publish runs
   only after the signed tarball is built and verified, so a successful release run
   should always include the signed asset.
2. Inspect the tarball locally to confirm it contains the signing attestation:

   ```bash
   curl -fsSL -o package.tgz \
     https://github.com/IvanMurzak/Unity-AI-ProBuilder/releases/download/<version>/com.ivanmurzak.unity.mcp.probuilder-<version>.tgz
